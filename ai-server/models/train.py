...
class ModelTrain:
    def __init__(self, config: dict) -> None:
        ...
        # 모델 불러오기
        if config["load_my_model"]: # load_my_model: bool
            try:
                self._tokenizer = AutoTokenizer.from_pretrained(self._model_name, local_file_only=True)  # 토크나이저
                ...
                raise ValueError("해당 모델이 존재하지 않습니다.")
        else:
...
if __name__ == "__main__":
    ...
    if not train_data_df.empty:
        ...
        train_config["coffee_review_valid_data"] = train_labels.count(1)
        train_config["not_coffee_review_valid_data"] = train_labels.count(0)
        print(f"커피 관련 리뷰 개수(검증): {train_labels.count(1)}\n커피 관련 없는 리뷰 개수(검증): {train_labels.count(0)}")
        ...
        model_trainer.train(train_dataset, valid_dataset, save_config=True, valid=True)
    else:
